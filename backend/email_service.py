...
class EmailService:
    ...
    async def send_admin_new_order_notification(self, order_data: dict):
        ...
        # Check if it's a custom manual payment gateway
        if payment_method.startswith('manual-'):
            payment_gateway_name = order_data.get('payment_gateway_name', 'Manual Payment')
            payment_display = f"💵 {payment_gateway_name}"
        else:
            payment_display = payment_method_display.get(payment_method, payment_method or 'N/A')
        
        # Add payment instructions section for manual payments
        payment_instructions_html = ""
        payment_gateway_instructions = order_data.get('payment_gateway_instructions', '')
        
        if (payment_method == 'manual' or payment_method.startswith('manual-')) and payment_gateway_instructions:
            payment_instructions_html = f"""
            <div style="margin-top: 20px; padding: 20px; background: #fff3cd; border-left: 4px solid #ffc107; border-radius: 5px;">
                <h3 style="color: #856404; margin-top: 0;">💰 Payment Instructions for Customer</h3>
                <div style="white-space: pre-wrap; margin: 15px 0; line-height: 1.6; color: #856404;">{payment_gateway_instructions}</div>
                <p style="margin: 10px 0; color: #856404;"><strong>Amount to Receive:</strong> <span style="font-size: 18px;">${order_data['total']:.2f}</span></p>
                <p style="background: #fff; padding: 10px; border-radius: 3px; margin-top: 15px; color: #856404;">
                    <strong>⚠️ Note:</strong> Customer should include order number <strong>{order_data['order_number']}</strong> as reference.
                </p>
            </div>
            """
